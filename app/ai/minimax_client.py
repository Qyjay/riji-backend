...
class MiniMaxClient:
    ...
    def _heuristic_emotion(cls, text: str) -> dict:
        ...
        if not content:
            return {"label": "平静", "score": 0, "emoji": "😌"}

        lowered = content.lower()
        rules = [
            ("难过", -7, "😢", ["想哭", "哭", "难过", "伤心", "悲伤", "委屈", "心碎", "崩溃", "低落", "沮丧", "失落", "难受", "emo"]),
            ("愤怒", -6, "😠", ["生气", "愤怒", "火大", "气死", "烦死", "暴躁", "恼火", "讨厌", "破防"]),
            ("焦虑", -5, "😟", ["焦虑", "紧张", "压力", "担心", "害怕", "慌", "忐忑", "不安", "睡不着"]),
            ("开心", 8, "😊", ["开心", "高兴", "快乐", "愉快", "幸福", "太棒", "好开心", "兴奋", "喜悦"]),
            ("感动", 6, "🥹", ["感动", "暖心", "泪目", "被治愈", "触动", "谢谢你", "温暖"]),
            ("期待", 6, "🤩", ["期待", "盼", "希望", "想要", "想去", "明天一定", "跃跃欲试"]),
            ("无聊", -2, "😑", ["无聊", "没劲", "空虚", "发呆", "不知道干嘛", "好闲"]),
            ("平静", 0, "😌", ["平静", "还行", "一般", "普通", "正常"]),
        ]
    ...
    def _normalize_emotion_payload(cls, payload: dict, fallback_text: str = "") -> dict:
        ...
        raw_score = (payload or {}).get("score", 0)
        try:
            score = float(raw_score)
        except Exception:
            score = float(cls._heuristic_emotion(fallback_text)["score"])

        if 0 < score <= 1 and not isinstance(raw_score, int):
            legacy_base = {
                "开心": 8,
                "期待": 6,
                "感动": 6,
            ...
            score = legacy_base * score
        elif 10 < score <= 100:
            score = (score - 50) / 5

        score = max(-10, min(10, int(round(score))))
    ...
    async def extract_emotion(self, text: str) -> dict:
        ...
        system = (
            "你是情绪分析助手。只分析用户文本中可见的主要情绪，严格返回 JSON，禁止输出解释、Markdown 或额外文字。\n"
            "JSON 格式：{\"label\":\"情绪名称\",\"score\":0,\"emoji\":\"单个表情\"}\n"
            "label 只能从以下 8 类选择：开心、难过、愤怒、平静、感动、焦虑、期待、无聊。\n"
            "score 是情绪好坏分，必须是 -10 到 10 的整数，越高表示主观体验越好，0 表示中性或无法判断。\n"
            "评分锚点：10=极强正向/幸福/狂喜；7~9=明显正向；4~6=轻到中度正向；1~3=微弱正向；"
            "0=平静、事实描述或正负抵消；-1~-3=轻微负向；-4~-6=中度负向；-7~-9=明显痛苦/愤怒/焦虑；-10=极端崩溃或绝望。\n"
            "label 表示情绪类型，score 表示好坏程度，二者不能混淆：惊讶、震惊不作为 label，必须根据上下文判断好坏；"
            "例如中奖后的惊讶可归为开心且 score 为 7~9，坏消息后的震惊可归为难过/焦虑且 score 为 -7~-9。\n"
            "不要把语气强烈程度当作高分，愤怒、焦虑、难过通常应为负分；平静通常为 -1 到 1。"
        )
