...
class MiniMaxClient:
    ...
    def _heuristic_emotion(cls, text: str) -> dict:
        ...
        rules = [
            ("难过", 0.9, "😢", ["想哭", "哭", "难过", "伤心", "悲伤", "委屈", "心碎", "崩溃", "低落", "沮丧", "失落", "难受", "emo"]),
            ("愤怒", 0.9, "😠", ["生气", "愤怒", "火大", "气死", "烦死", "暴躁", "恼火", "讨厌", "破防"]),
            ("焦虑", 0.85, "😟", ["焦虑", "紧张", "压力", "担心", "害怕", "慌", "忐忑", "不安", "睡不着"]),
            ("开心", 0.9, "😊", ["开心", "高兴", "快乐", "愉快", "幸福", "太棒", "好开心", "兴奋", "喜悦"]),
            ("感动", 0.85, "🥹", ["感动", "暖心", "泪目", "被治愈", "触动", "谢谢你", "温暖"]),
            ("期待", 0.85, "🤩", ["期待", "盼", "希望", "想要", "想去", "明天一定", "跃跃欲试"]),
            ("无聊", 0.75, "😑", ["无聊", "没劲", "空虚", "发呆", "不知道干嘛", "好闲"]),
            ("平静", 0.7, "😌", ["平静", "还行", "一般", "普通", "正常"]),
        ]
    ...
    def _normalize_emotion_payload(cls, payload: dict, fallback_text: str = "") -> dict:
        ...
        # 对外契约是 0-1 的识别置信度；兼容模型返回百分制和旧版 -10~10 强度。
        if score > 10:
            score = score / 100
    ...
    async def extract_emotion(self, text: str) -> dict:
        ...
        system = (
            ...
            "JSON 格式：{\"label\":\"情绪名称\",\"score\":0.85,\"emoji\":\"单个表情\"}\n"
            "label 只能从以下 8 类选择：开心、难过、愤怒、平静、感动、焦虑、期待、无聊。\n"
            "score 表示判断该标签的置信度，必须是 0 到 1 的小数，越高表示证据越明确。\n"
            "label 表示情绪类型，score 不表示情绪正负或强度；无法判断时使用平静且 score 不高于 0.5。"
        )
