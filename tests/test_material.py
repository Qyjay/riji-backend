...
def test_create_material_with_multiple_uploaded_images(client):
    ...
    user_data = create_test_user(client, username="material_multi_image_user")
    headers = get_auth_header(user_data["token"])
    ...
    assert data["thumbnailUrl"] == thumbnail1
    assert data["location"] == location1


def test_batch_upload_diary_images_flow(client):
    """批量上传接口可一次上传多张并用于创建素材。"""
    user_data = create_test_user(client, username="material_batch_upload_user")
    headers = get_auth_header(user_data["token"])
...
def test_batch_upload_diary_images_limit(client):
    """批量上传超过上限时应返回 400。"""
    user_data = create_test_user(client, username="material_batch_upload_limit")
    headers = get_auth_header(user_data["token"])
