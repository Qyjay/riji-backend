...
后端现在**已经有统一记忆底座**，并且记忆、聊天、分身、广场之间已经出现了真实联动；当前阶段不再是“从 0 到 1 设计 memory”，而是“把现有 memory + RAG + 分身链路打磨稳定”。

| 模块 | 现有能力 | 记忆系统中的角色 |
| --- | --- | --- |
| `app/chat` | AI 对话、会话段、聊天历史、自动转素材、RAG 注入 | 对话记忆来源、长期聊天上下文、聊天场景 RAG |
| `app/material` | 原始素材，如文字、图片、语音 | 多模态生活素材入口 |
| `app/diary` | 日记生成、日记内容、情绪、标签、搜索 | 高价值自我叙事记忆 |
| `app/plaza` | 广场帖子、评论、分身评论草稿 | 公开表达、社交意图、广场公共记忆 |
| `app/social` | 匹配、私聊、匹配报告 | 人际互动记忆、关系演化 |
| `app/avatar` | 分身记忆、分身侧写、分身状态、分身匹配、分身名片、行动审批 | 分身运行层 |
| `app/memory` | 统一记忆文档、切块、检索、抽取、画像、隐私边界 | 统一记忆底座 |

当前已经落地的关键能力：

- 已有统一 `MemoryDocument / MemoryChunk / MemoryFact / MemoryProfile / AvatarCard / AgentAction`
- 已能把日记、素材、聊天、广场帖子/评论、社交私聊沉淀到统一记忆系统
- 已支持 SQLite fallback 检索，且可选接入 ChromaDB 向量索引
- RAG 已融入 AI 对话
- RAG 已开始融入分身侧写、分身推荐和分身评论草稿
- 已有 agent-to-agent 的安全上下文输出，默认不暴露 private 原文

当前主要缺口：

- 需要继续验证“所有来源真实写入向量库”的稳定性与可观测性
- `AvatarProfile` 与统一 `MemoryProfile` 还有进一步收敛空间
- 分身更多行为仍在“建议/草稿优先”的阶段，自动化程度需要谨慎推进
- 召回质量、向量健康状态、全量回归测试还需要继续打磨
